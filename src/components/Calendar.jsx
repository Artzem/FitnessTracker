...
export default function Calendar() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const workouts = await loadAllWorkoutProgress(currentUser.uid)
        const foods = await loadAllFoodProgress(currentUser.uid)
  ...
  const getProgressForDate = (dateKey) => {
    ...
    // 2. Calculate Food Progress (FIXED)
    if (food) {
      let totalCals = 0;
      // Loop through all meals to find eaten items
      ...
      const goal = Number(food.calorieGoal) || 2000
      if (totalCals > 0) {
                  ...
                  >
                    <div className="absolute inset-0 p-2 flex flex-col">
                      ...
                      {workout && workout.workout && (
                        <div className="text-xs text-orange-300 font-semibold mb-1 truncate px-1 rounded bg-black/20">
                          {workout.workout}
                        </div>
                      )}
